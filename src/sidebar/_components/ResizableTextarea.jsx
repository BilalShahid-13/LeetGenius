...
const ResizableTextarea = ({ onChange, onKeyDown, value, handleSend, onLoading }) => {
  ...
  return (
    ...
    border border-yellow-400 rounded-lg px-3 py-2 resize-none focus:outline-none
    bg-gray-200 dark:bg-zinc-800 focus:ring-2 focus:ring-yellow-500">
      <Button variant="outline" onClick={handlePaste}
        className="dark:bg-zinc-900 bg-gray-300 dark:hover:bg-zinc-950 rounded-full px-[10px] py-4">
        <Clipboard className="dark:text-gray-200 text-zinc-950" />
      </Button>
      <TextareaAutosize
        disabled={onLoading}
        minRows={1}
      ...
      <Button variant="outline" onClick={handleSend}
        ...
        {onLoading ?
          ...
          <div role="status">
            <svg aria-hidden="true" class="w-full h-full object-cover text-gray-200 animate-spin dark:text-yellow-500 fill-yellow-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          : <Send className="dark:text-gray-200 text-zinc-950" />}
      </Button>
    </div>
  );
};
